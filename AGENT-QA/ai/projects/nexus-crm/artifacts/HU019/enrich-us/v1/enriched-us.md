# HU enriquecida - HU019: Consultar indicadores generales del CRM
...
| Contexto breve | Consultar indicadores generales del CRM. |
| Prioridad | 2 |
| Version | v1 |
...
| HU ID | HU019 |
| Work Item Azure DevOps | 479 |
| Titulo origen | HU019 - Consultar indicadores generales del CRM |
...
Quiero visualizar indicadores consolidados sobre empresas, contactos, actividades y calidad de datos  
Para conocer el estado general de la gestión comercial.
...
La HU HU019 pertenece al backlog funcional de Nexus CRM y cubre la capacidad 'Consultar indicadores generales del CRM'. La version enriquecida conserva la intencion original leida desde Azure DevOps y agrega criterios verificables para refinamiento, QA y validacion de negocio.
...
Cuando solicita consultar indicadores generales del crm  
...
Cuando completa la informacion requerida para consultar indicadores generales del crm  
Entonces el sistema procesa la operacion  
Y confirma el resultado de forma clara.

### CA-003 - Validar informacion obligatoria

Dado que el usuario esta ejecutando consultar indicadores generales del crm  
...
Dado que un usuario no autorizado intenta consultar indicadores generales del crm  
...
Dado que la operacion consultar indicadores generales del crm se completa correctamente  
...
### CA-006 - Validar alcance: Total empresas

Dado que el alcance de la HU incluye total empresas  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar total empresas segun las reglas aprobadas por negocio.

### CA-007 - Validar alcance: Total contactos

Dado que el alcance de la HU incluye total contactos  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar total contactos segun las reglas aprobadas por negocio.

### CA-008 - Validar alcance: Sin seguimiento

Dado que el alcance de la HU incluye sin seguimiento  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar sin seguimiento segun las reglas aprobadas por negocio.

### CA-009 - Validar alcance: Duplicados pendientes
...
- La HU fue leida desde Azure DevOps como Work Item 479.
- El titulo funcional es HU019 - Consultar indicadores generales del CRM.
- El estado actual en Azure DevOps es New.
- La HU debe mantener trazabilidad con el proyecto Nexus.
- El alcance indicado incluye Total empresas.
- El alcance indicado incluye Total contactos.
- El alcance indicado incluye Sin seguimiento.
- El alcance indicado incluye Duplicados pendientes.
- El alcance indicado incluye Correos corporativos.
