# HU enriquecida - HU015: Ejecutar y confirmar importación
...
| Contexto breve | Ejecutar y confirmar importación. |
| Prioridad | 2 |
| Version | v1 |
...
| HU ID | HU015 |
| Work Item Azure DevOps | 473 |
| Titulo origen | HU015 - Ejecutar y confirmar importación |
...
Quiero visualizar un resumen del proceso y confirmar la importación de los registros válidos  
Para incorporar la información al CRM de forma controlada y verificable.
...
La HU HU015 pertenece al backlog funcional de Nexus CRM y cubre la capacidad 'Ejecutar y confirmar importación'. La version enriquecida conserva la intencion original leida desde Azure DevOps y agrega criterios verificables para refinamiento, QA y validacion de negocio.
...
Cuando solicita ejecutar y confirmar importación  
...
Cuando completa la informacion requerida para ejecutar y confirmar importación  
Entonces el sistema procesa la operacion  
Y confirma el resultado de forma clara.

### CA-003 - Validar informacion obligatoria

Dado que el usuario esta ejecutando ejecutar y confirmar importación  
...
Dado que un usuario no autorizado intenta ejecutar y confirmar importación  
...
Dado que la operacion ejecutar y confirmar importación se completa correctamente  
...
### CA-006 - Validar alcance: Total de filas procesadas

Dado que el alcance de la HU incluye total de filas procesadas  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar total de filas procesadas segun las reglas aprobadas por negocio.

### CA-007 - Validar alcance: Registros a importar

Dado que el alcance de la HU incluye registros a importar  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar registros a importar segun las reglas aprobadas por negocio.

### CA-008 - Validar alcance: Duplicados encontrados

Dado que el alcance de la HU incluye duplicados encontrados  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar duplicados encontrados segun las reglas aprobadas por negocio.

### CA-009 - Validar alcance: Datos incompletos

Dado que el alcance de la HU incluye datos incompletos  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar datos incompletos segun las reglas aprobadas por negocio.

## Reglas de negocio

### Reglas confirmadas por HU o contexto

- La HU fue leida desde Azure DevOps como Work Item 473.
- El titulo funcional es HU015 - Ejecutar y confirmar importación.
- El estado actual en Azure DevOps es New.
- La HU debe mantener trazabilidad con el proyecto Nexus.
- El alcance indicado incluye Total de filas procesadas.
- El alcance indicado incluye Registros a importar.
- El alcance indicado incluye Duplicados encontrados.
- El alcance indicado incluye Datos incompletos.
- El alcance indicado incluye Empresas nuevas.
