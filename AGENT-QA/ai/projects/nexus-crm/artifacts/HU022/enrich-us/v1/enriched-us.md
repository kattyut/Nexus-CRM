# HU enriquecida - HU022: Consultar indicadores estratégicos
...
| Contexto breve | Consultar indicadores estratégicos. |
| Prioridad | 2 |
| Version | v1 |
...
| HU ID | HU022 |
| Work Item Azure DevOps | 488 |
| Titulo origen | HU022 - Consultar indicadores estratégicos |
...
Quiero visualizar indicadores estratégicos sobre empresas, contactos y actividad comercial  
Para identificar oportunidades y riesgos en la gestión comercial.
...
La HU HU022 pertenece al backlog funcional de Nexus CRM y cubre la capacidad 'Consultar indicadores estratégicos'. La version enriquecida conserva la intencion original leida desde Azure DevOps y agrega criterios verificables para refinamiento, QA y validacion de negocio.
...
Cuando solicita consultar indicadores estratégicos  
...
Cuando completa la informacion requerida para consultar indicadores estratégicos  
Entonces el sistema procesa la operacion  
Y confirma el resultado de forma clara.

### CA-003 - Validar informacion obligatoria

Dado que el usuario esta ejecutando consultar indicadores estratégicos  
...
Dado que un usuario no autorizado intenta consultar indicadores estratégicos  
...
Dado que la operacion consultar indicadores estratégicos se completa correctamente  
...
### CA-006 - Validar alcance: Empresas abandonadas

Dado que el alcance de la HU incluye empresas abandonadas  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar empresas abandonadas segun las reglas aprobadas por negocio.
...
### CA-009 - Validar alcance: Alta cobertura

Dado que el alcance de la HU incluye alta cobertura  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar alta cobertura segun las reglas aprobadas por negocio.

## Reglas de negocio

### Reglas confirmadas por HU o contexto

- La HU fue leida desde Azure DevOps como Work Item 488.
- El titulo funcional es HU022 - Consultar indicadores estratégicos.
- El estado actual en Azure DevOps es New.
- La HU debe mantener trazabilidad con el proyecto Nexus.
- El alcance indicado incluye Empresas abandonadas.
- El alcance indicado incluye Contactos saturados.
- El alcance indicado incluye Sin seguimiento.
- El alcance indicado incluye Alta cobertura.
- El alcance indicado incluye KPIs destacados.
