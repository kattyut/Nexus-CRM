# HU enriquecida - HU021: Consultar actividad reciente y alertas operativas
...
| Contexto breve | Consultar actividad reciente y alertas operativas. |
| Prioridad | 2 |
| Version | v1 |
...
| HU ID | HU021 |
| Work Item Azure DevOps | 483 |
| Titulo origen | HU021 - Consultar actividad reciente y alertas operativas |
...
Quiero visualizar las actividades recientes y las alertas relevantes del sistema  
Para priorizar acciones y realizar seguimiento oportuno.
...
La HU HU021 pertenece al backlog funcional de Nexus CRM y cubre la capacidad 'Consultar actividad reciente y alertas operativas'. La version enriquecida conserva la intencion original leida desde Azure DevOps y agrega criterios verificables para refinamiento, QA y validacion de negocio.
...
Cuando solicita consultar actividad reciente y alertas operativas  
...
Cuando completa la informacion requerida para consultar actividad reciente y alertas operativas  
Entonces el sistema procesa la operacion  
Y confirma el resultado de forma clara.

### CA-003 - Validar informacion obligatoria

Dado que el usuario esta ejecutando consultar actividad reciente y alertas operativas  
...
Dado que un usuario no autorizado intenta consultar actividad reciente y alertas operativas  
...
Dado que la operacion consultar actividad reciente y alertas operativas se completa correctamente  
...
### CA-006 - Validar alcance: Actividad reciente

Dado que el alcance de la HU incluye actividad reciente  
Cuando el usuario ejecuta el flujo correspondiente  
Entonces el sistema permite validar actividad reciente segun las reglas aprobadas por negocio.
...
### CA-008 - Validar alcance: Contactos saturados
...
- La HU fue leida desde Azure DevOps como Work Item 483.
- El titulo funcional es HU021 - Consultar actividad reciente y alertas operativas.
- El estado actual en Azure DevOps es New.
- La HU debe mantener trazabilidad con el proyecto Nexus.
- El alcance indicado incluye Actividad reciente.
...
- El alcance indicado incluye Acceso a detalle.
